...
def main(): 
    ...
    def busqueda():
        ...
        if genero == "<Cualquiera>" and valoracion == "<Cualquiera>":
            for pelicula in peliculas:
                if nombre.lower() in pelicula[0] or nombre.lower() in pelicula[1]:
                    print(pelicula)

    #archivo peliculas
    with open("peliculas.csv", "r", encoding= "utf-8") as archivo_peliculas:
        peliculas = crea_lineas(archivo_peliculas)
    #archivo generos
    with open("generos.csv","r",encoding="utf-8") as archivo_generos:
    ...
    search_button = Button(search_frame,image=search_icon,background=LIGHT_COLOR,border=0,cursor="hand2",command=busqueda)
